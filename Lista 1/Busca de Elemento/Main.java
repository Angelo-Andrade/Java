        ...
        while (true) {
            try {
                ...
                if (input.equals("\\s")) {
                    ...
                    int index = Arrays.binarySearch(cities, search);
                    if (index >= 0) {
                        System.out.println("O elemento procurado está na posição: " + index);
                    } else {
                        System.out.println("Elemento não encontrado.");
                    }
                }
                else if (input.isEmpty() || Arrays.asList(cities).contains(input)) {
